...
- **Composition.** SegmentedControl reports the choice through onValueChange and the example keeps it in state; both lists are written out in the markup, each in its own Nav, and the one not chosen is hidden on an element of the example's own, so Nav knows nothing of the switch.
- **Accessible & gatekept.** The radio group is named Section by a legend that is read but not seen, each nav is named for its section so a landmark list says which is shown, and the current page stays marked in the Account list where it lives.
