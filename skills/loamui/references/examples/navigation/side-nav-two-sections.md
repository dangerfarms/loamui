...
- **Composition.** Two Lists in one Root: Nav has no footer part, because a second List, a wrapper and two align-self values are all the pinning needs; the example writes nothing onto Nav's grid and never reaches inside a link, the sign-out button included.
...
/* Nav's grid stretches its auto rows equally: the wrapper keeps the title
   and the main list in one row, pinned to its start, and the account list
   is pinned to the end of the other. */
