...
Your agent first checks the package, stylesheet and existing tools. If anything is missing, it proposes the setup needed for your request, reuses your configuration and helps complete the agreed changes before building. If the project is ready, it proceeds directly. You do not need a separate preparation prompt or to configure each tool by hand.
...
This runs the same setup assessment used by a build request. Your agent proposes any missing tools or shared configuration changes and runs the agreed checks.
