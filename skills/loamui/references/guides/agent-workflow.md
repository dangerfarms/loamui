...
**Before you start:** create your framework application, install LoamUI and import its stylesheet using [Installation](/docs/installation). Installing the skill does not install LoamUI. Your agent can help complete missing setup when you ask it to.
...
Your agent checks the package, stylesheet and existing tools. It proposes missing setup before making shared changes, reuses your configuration, and runs the agreed checks. If the project is already ready, it moves on. You do not need to configure each tool by hand.
...
You can also choose a [recipe](/recipes) and copy the short prompt shown on its card. Add the content or behaviour you want to change; you do not need to list CSS techniques or accessibility rules in the prompt.
...
If your tool cannot install a skill, provide [llms.txt](/llms.txt) and a recipe prompt. When it cannot open links, attach the **Full reference** linked on the recipe card. A chat preview needs access to the real package and stylesheet; when that is unavailable, ask for source files to use in your configured application. Preview and runtime behaviour still need checking there.
...
```bash
npm install --save-dev postcss postcss-value-parser typescript
```
...
The LoamUI skill carries the essential composition rules and worked examples. Users do not need to invoke companion skills separately. When the coding agent supports skills, offer these project-local additions using the supported installer and the active agent's target:
...
Read the [recipe guide](/recipes/guide), the selected recipe's complete TSX and CSS, and the reference for each core component used. Skill users can read these offline in `references/`. Each recipe card shows a short prompt that names the LoamUI skill and recipe. Resolve the name through the bundled reference index; do not ask the user to restate the implementation rules. The card also links to a complete reference for assistants without skills or browsing: it includes the selected source, workflow and component contracts.
