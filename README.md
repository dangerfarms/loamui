...
5. **Accessible & gatekept.** Semantic HTML, managed focus, keyboard support,
   ...
   palette is contrast-audited in CI and every component ships
   [axe](https://github.com/dequelabs/axe-core) and keyboard tests. What a tool
   can verify, a tool verifies.
...
- **API.** No `size`, `variant`, `color`, or `fullWidth` props: size comes from
  ...
  sizes to its content, and Drawer's panel width is the public
  `--loam-drawer-size` property.) A component is named for the HTML element
  it's built on, not a design-system alias — `Range` (`<input type="range">`),
  not `Slider`; `Details` (`<details>`), not `Accordion`.
...
- **CSS.** Selectors are `@scope`d, not BEM — one `loam-` class per root, parts
  ...
  with a donut (`to ([class*="loam-"])`). Refer to elements directly — no
...
them through the `.claude/skills/` symlinks.
