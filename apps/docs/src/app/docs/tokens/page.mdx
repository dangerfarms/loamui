...
The first of FarmUI's three primitives: a handful of semantic custom properties that every component reads. A small palette of colours and neutrals, plus fluid scales: everything else is derived by recipe, so theming is overriding `--fui-*` values at any scope. No `ThemeProvider`, no JavaScript.
...
One token carries the brand: `--fui-color-primary`. Everything else (the soft tint, hover, active,
solid fills, focus rings) is _derived_ from it by recipe, so there is nothing else to keep in
sync:

```css
:root {
  --fui-color-primary: light-dark(oklch(0.62 0.2 275), oklch(0.72 0.17 275)); /* violet */
}
```
...
<div style={{ "--fui-context": "primary" }}>
  <Button>Default brand</Button>   {/* --fui-color-primary resolves at :root */}
</div>

<div style={{
  "--fui-color-primary": "light-dark(oklch(0.62 0.2 275), oklch(0.72 0.17 275))",
  "--fui-context": "primary",
}}>
...
`color: var(--fui-color-fg)`) for descendants to pick up the flipped value.
...
- `--fui-color-primary`: the brand colour (the soft tint, solid fill, focus ring and hover/active
  states all derive; there is nothing else to sync)
- `--fui-color-bg`, `--fui-color-surface`, `--fui-color-fg`, `--fui-color-line`: surfaces & text
- `--fui-radius-md`, `--fui-radius-lg`: corner rounding
- `--fui-font`, `--fui-font-display`: the body and heading font families (both `system-ui` by
...
@layer farmui.tokens, farmui.elements, farmui.components, app;
