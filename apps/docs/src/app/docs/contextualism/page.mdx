...
The platform itself is a detection source too. Fields also match `:user-invalid`,
so native constraint validation (`required`, `type="email"`)
styles the field after the user interacts: no error prop, no state, no JavaScript at all.
And an _icon-only_ button is detected from its accessible name: the
