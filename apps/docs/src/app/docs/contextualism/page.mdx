...
In most component libraries you tell each component what to look like: `variant="outline"`, `size="lg"`, `color="danger"`. FarmUI inverts that. Components read their _context_ (what the surrounding region means, how much space it has, what the component contains) and adapt themselves. Identity props are the last resort, not the default.
...
- **What the region means.** `--fui-context`: `primary` (the action of the area), `danger`
  (destructive territory), and the statuses `success`, `warning` and `info`.
- **The size of the space.** Container queries and fluid `cqi` tokens; no size props.
...
Notice the checkbox: `--fui-context` is not a button feature. The region remaps
the semantic colour tokens for _every_ FarmUI component inside: checked states, focus
...
```css
.fui-Button {
  --_color: var(--fui-button-color, var(--fui-color-fg));

  border: 1px solid color-mix(in oklab, var(--_color), var(--fui-color-bg) 80%);
  background: light-dark(
    color-mix(in oklab, var(--_color), var(--fui-color-bg) 90%),
    color-mix(in oklab, var(--_color), var(--fui-color-bg) 75%)
  );
...
The single instance-level escape hatch is the registered `--fui-button-color`
...
When the DOM already expresses a state, FarmUI styles it with `:has()` instead of
...
specialize with a wrapper component (or the `--fui-button-color` channel), and
...
`--fui-context`) became Baseline Newly Available in May 2026, when the last
...
[browser policy](https://github.com/dangerfarms/farmui/blob/main/CONTRIBUTING.md)
