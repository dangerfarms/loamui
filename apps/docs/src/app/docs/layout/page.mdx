...
export const box = {
  background: "var(--fui-color-primary-soft)",
...
<div className={prose.block}>
  <div style={{ ...stage, display: "flex", flexWrap: "wrap", gap: "var(--fui-space-sm)" }}>
...
gap: var(--fui-space-xs); /* 4px  → 4.5px  */
gap: var(--fui-space-sm); /* 8px  → 9px    */
gap: var(--fui-space-md); /* 12px → 13.5px */
gap: var(--fui-space-lg); /* 16px → 18px   */
gap: var(--fui-space-xl); /* 24px → 27px   */
```
