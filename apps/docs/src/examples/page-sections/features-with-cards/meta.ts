...
export const meta: ExampleMeta = {
  ...
  notes: {
    native:
      "A section named by its h2, a header for the intro and a list of six cards with h3 headings, so the outline reads the way the page looks.",
    modern:
      "The list is an auto-fit grid answering the section's own width, and each card is scoped from its own root so the section's rule never reaches inside a Card.",
    composition:
    ...
    context:
      "The icon square is the soft primary tint with the strong token for the glyph, so a --loam-context region around the section re-colours every icon at once.",
    accessible:
