...
export const meta: ExampleMeta = {
  ...
  notes: {
    ...
    composition:
      "Nav is core's, with its own scope; the header asks one thing of it, a flex row on the List, and places the nav from a scope of its own rather than reaching through the donut.",
    accessible:
      "The current page is marked by aria-current on the link, and the stylesheet draws the marker from that attribute, so a router and a static site say it the same way.",
  },
