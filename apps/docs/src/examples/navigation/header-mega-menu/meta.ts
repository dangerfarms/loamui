...
export const meta: ExampleMeta = {
  ...
  notes: {
    native:
      "The panel is a native popover in the top layer, so light dismiss and Escape are the browser's, and every guide inside is an anchor in a list; the panel opens on a press, not on hover, so a keyboard and a touch screen open the same thing.",
    modern:
      "The panel's width is Nav's public --loam-nav-dropdown-size, raised to 32rem on the Root where the panel reads it, and the guides fill it with auto-fill columns, so two sit side by side where there is room and one where there is not; the strip at the foot is a box with its own padding inside the panel's.",
    composition:
