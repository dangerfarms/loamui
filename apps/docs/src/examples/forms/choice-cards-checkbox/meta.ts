...
export const meta: ExampleMeta = {
  ...
  notes: {
    native:
      "A fieldset names the set and each card is a <label> around a native checkbox sharing one name, so the form submits every ticked value under it and the whole surface is the click target.",
    modern:
      "Checked and disabled are detected from the input with :has(), never declared on the card; the disabled option fades its words while the Card keeps its line, so it still reads as one of the set.",
    composition:
    ...
    context:
      "The checked edge uses the -strong primary so it holds 3:1 in both schemes, and moves to the system Highlight in forced colours, where the disabled words go to GrayText instead of fading.",
    accessible:
