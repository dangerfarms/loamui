...
export const meta: ExampleMeta = {
  ...
  notes: {
    native:
      "The link is in a real read-only input, so it can be focused, selected and copied with the keyboard whether or not the Clipboard API is available; focusing it selects the whole address, one keystroke from a manual copy.",
    modern:
      "The row is a two-track grid; the CopyButton's wrapper has no box, so the Button takes the second track as a bare Button would, sharing the derived control height with the box beside it.",
    composition:
