...
const accountForm: Composition = {
  ...
  lead: "Seven parts in a wrapper capped at 24rem and centred, on a core Card or on no surface at all. Every form at the door of an account has the same shape: a title the form is named by, one column of fields, a Button stretched to the form's width that says what happens, and a footer line for the other path. The questions differ, and the answers live in the fields' attributes and copy, so the three forms are three recipes on one composition rather than three components.",
  importLine: `import { AccountForm } from "@loamui/ui";\nimport { Alert, Button, Checkbox, ErrorSummary, Field, Input, Modal } from "@loamui/core";`,
  parts: [
    {
      name: "AccountForm.Root",
      description:
        "The unit: a wrapper that caps the form at 24rem and centres it in whatever holds it, and declares its own container so the fluid tokens answer the form's width. It mints the name the Form wears and the Title renders, so the server HTML already carries it.",
    },
    {
      name: "AccountForm.Card",
      description:
        "The surface: core Card, left as core styles it. Put the parts inside it on a page of its own; leave it out where the form already has a surface, inside a Modal.Popup say, and put the parts straight in the Root.",
    },
    {
      name: "AccountForm.Title",
      description:
        "The heading, and the form's accessible name: the Form points at it with aria-labelledby from the first render. An h1 on a page of its own; pass render={<h2 />} inside a page, and give it an id of your own when something else must point at it.",
    },
    {
    ...
    },
    {
      ...
      description:
        "The native form, one column of fields with a large gap, named by the Title; your own aria-label or aria-labelledby wins. action and onSubmit are its own, forwarded untouched. After a failed submit, an ErrorSummary goes here as the first child, spaced like any other row.",
    },
    {
      name: "AccountForm.Actions",
      description: "A single-cell grid for the submit Button, so it stretches to the form's width.",
    },
    {
  ...
  <AccountForm.Card>
    ...
    },
    {
      title: "In a dialog",
      description:
        "A sign-in asked for in place, over the page, has a surface already: the Modal.Popup. So the Card is left out and the parts go straight in the Root, which still caps and centres them. The Title is an h2 with an id of its own, and the Popup names itself by it with aria-labelledby, so the dialog and the form share one name; Escape and the Close button put focus back on the trigger, as every core Modal does.",
      code: `<Modal.Root>
